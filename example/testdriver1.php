...
use PHPBounceHandler\BounceHandler;
...
<P>
If the bounce is not well formed, it tries to extract some useful information anyway.  Currently Postfix and Exim are supported, partially.  You can edit the function <code>getTheFacts()</code> if you want to add a parser for your own busted MTA.</P>
<?
...
if($_GET['eml']){
    ...
    echo "<P>Quick and dirty bounce handler:<BR>
        ...
        \$multiArray = \$bouncehandler->getTheFacts(\$strEmail);  , or<br>
        \$multiArray = \$bouncehandler->parseEmail(\$strEmail);</code>  (same thing)
        </blockquote>
...
//print_r($bouncehandler); exit;
    ...
    $bounce = $bouncehandler->initBounceHandler($bounce);
...
$head_hash = $bouncehandler->parseHead($head);
...
if ($bouncehandler->isRFC1892MultipartReport($head_hash)){
...
$mime_sections = $bouncehandler->parseBodyIntoMimeSections($body, $boundary);
$rpt_hash = $bouncehandler->parseMachineParsableBodyPart($mime_sections['machine_parsable_body_part']);
...
for($i=0; $i<count($rpt_hash['per_recipient']); $i++){
    ...
    echo $bouncehandler->findRecipient($rpt_hash['per_recipient'][$i]);
    $scode = $rpt_hash['per_recipient'][$i]['Status'];
    echo "<PRE>$scode</PRE>";
    echo $bouncehandler->fetchStatusMessages($scode);
    echo "</P>\n";
}
...
for($i=0; $i<count($rpt_hash['per_recipient']); $i++){
    ...
    if($dcode){
        echo "<PRE>$dcode</PRE>";
        echo $bouncehandler->fetchStatusMessages($dcode);
    }
    else{
...
$head = $bouncehandler->getHeadFromReturnedMessageBodyPart($mime_sections);
